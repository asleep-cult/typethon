...
# overhaul, this is based on CPython grammar and
# its very inefficient without the optimizations
...
class Parser:
    ...
    def optional_lookahead(
        ...
        *types: TokenType,
        ...
        with self.lookahead(*types, negative=negative) as alternative:
            result = function()
    ...
    def class_def(
        ...
        *,
        ...
        assert token.type is TokenType.CLASS
    ...
    def function_def(
        ...
        *,
        ...
        token = self.stream.consume_token()
        assert token.type is TokenType.DEF
        ...
        token = self.stream.peek_token()
        if token.type is not TokenType.OPENPAREN:
    ...
    def parameters(self) -> typing.List[ast.FunctionParameterNode]:
        ...
        while True:
            token = self.stream.peek_token()
            if token.type is TokenType.SLASH:
    ...
    def parameter(self) -> ast.FunctionParameterNode:
        ...
        token = self.stream.peek_token()
        if token.type is TokenType.EQUAL:
            self.stream.consume_token()
    ...
    def simple_statement(self) -> ast.StatementNode:
        ...
        elif token.type is TokenType.YIELD:
    ...
    def type_atom(self) -> ast.TypeExpressionNode:
        ...
        elif token.type is TokenType.VERTICALBAR:
            with self.alternative():
    ...
    def type_parameter(self) -> ast.TypeParameterNode:
        token = self.stream.peek_token()
        assert token.type is TokenType.VERTICALBAR
    ...
    def dict_type(self) -> ast.DictTypeNode:
        token = self.stream.peek_token()
        if token.type is not TokenType.OPENBRACE:
